 ...
 *
 * 펜 overlay: 카드의 손글씨(card.overlay)를 본문 위에 같이 렌더한다. 좌표는
 * 카드 content box 기준 절대 px이며 스케일하지 않는다(카드 리사이즈가 펜을
 * 늘리지 않는 것과 동일) — 큰 모달에선 좌상단 원래 크기로 보인다. 헤더의
 * 펜/지우개 토글로 그리기 모드(active)를 켜면 그 위에 덧그리거나 지울 수 있고,
 * 꺼두면 SVG가 pointer-events:none → 클릭이 본문(텍스트 편집)으로 통과한다.
 * 변경은 setOverlay로 카드와 동일한 overlay 데이터에 영속된다.
 *
