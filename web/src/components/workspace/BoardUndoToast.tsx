...
/**
 * FEAT-boards §8: 보드 삭제 후 5초 동안 노출되는 undo toast.
 * pendingBoardUndo가 set되어 있을 때만 렌더되고, 만료 시 store가 자동으로 null로 비운다.
 */
export function BoardUndoToast() {
  const t = useT();
  const pending = useWorkspace((s) => s.pendingBoardUndo);
  const undo = useWorkspace((s) => s.undoBoardRemove);
  const clear = useWorkspace((s) => s.clearBoardUndo);
  const [remaining, setRemaining] = useState(BOARD_UNDO_MS);

  // 카운트다운(시각적) — 100ms 단위로 갱신, 만료 시 0으로
  useEffect(() => {
    if (!pending) return;
    const tick = () => {
  ...
  return (
    ...
    >
      <span className="text-text-muted">
        {t("workspace.boardPicker.deletedToast")} · {secondsLeft}s
      </span>
      <button
        type="button"
        onClick={() => void undo()}
