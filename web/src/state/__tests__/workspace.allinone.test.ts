...
import { parseBlocks, serializeCode, serializeHandwriting } from "@/state/cardContent";
...
describe("decodeNoteToCard · 올인원 마이그레이션", () => {
  ...
  it("lang 없는 code 노트도 손실 없이 환원", () => {
    ...
    const card = __internal.decodeNoteToCard(n);
    expect(parseBlocks(card.content)).toEqual([{ type: "code", code: "x=1" }]);
  });
  ...
  it("handwriting 노트 → text 카드 + 단일 handwriting 블록 (획 좌표 보존)", () => {
    ...
    expect(card.kind).toBe("text");
    expect(parseBlocks(card.content)).toEqual([{ type: "handwriting", paths }]);
  });
