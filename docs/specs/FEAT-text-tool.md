...
| Markdown 내보내기 | 텍스트 1개 = `.md` 1개 (2026-09-25) | 현 markdownExport엔 보드 단위 문서가 없다. 보드 문서를 새로 만들면 내보내기 구조 전체가 바뀐다 |
...
- **Then** JSON Canvas는 `type:"text"` 노드, Markdown은 텍스트 하나당 `.md` 파일 하나(메모와 같은 1장=1파일 구조 — 보드 단위 문서가 없어 2026-09-25 스펙 수정), .moss는 왕복 후 크기·색·폭 모드가 보존된다.
