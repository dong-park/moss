...
**deps**: n3-migration, n5-memo-front, n6-canvas-capture, n8-dock, n9-naming
**상태**: pending
...
2. `web/src/components/workspace/cards/{image,link,audio,file,mindmap}/Content.tsx`: 이관(n3) 후 렌더 대상이 없으면 삭제. 옛 백업 가져오기 경로가 여전히 쓰면 남기고 이유를 `구현 메모`에. `NoteKind` 값은 타입에 남긴다(spec §6).
3. 수동 시나리오 A: 빈 DB → 독에서 메모판·메모·파일함 생성 → 메모를 판에 넣고 판 이동 → 메모 창에서 이미지·링크·녹음·파일 블록 추가 → 앞면 배지 확인 → 새로고침.
4. 수동 시나리오 B: 옛 카드가 든 DB 사본(v4)으로 앱을 열어 이관 결과를 눈으로 확인 → `rollbackStickyMigration()` 실행 확인.
...
- [ ] plan 공통 완료 기준 전부
- [ ] 1번 grep에서 옛 종류를 **생성**하는 분기 0건(타입·이관·렌더 호환 참조는 허용, 남긴 것 목록을 `구현 메모`에)
