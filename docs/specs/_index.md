...
| squad | 보드 | 단위 | 상태 |
|---|---|---|---|
| 메모 툴 프로덕션화 | [_squad-memo-production.md](_squad-memo-production.md) | P0(에디터 슬롯화) + W1~W10 | ✅ **완료·ship** (2026-06-04) — 합본 게이트 그린(tsc 0·eslint 0·vitest 673), `feat/memo-editor-seams` → main 머지(PR #2). |
| 메모/펜 후속 | [_squad-memo-pen-followup.md](_squad-memo-pen-followup.md) | WS-A/B/C/D (펜 엔진·UX·학습성·정리) | ✅ **완료** (2026-06-05) — main 머지(`f9d43e8` 등), 브라우저 QA PASS. ⚠️ 후속 small fix 1건(펜 모드 Esc 전용 종료 → [[FEAT-pen-mode-ux]] §0). |

> **메모 툴 프로덕션화** — 에디터 사용성 10개 액션(자동저장 가드·이미지 붙여넣기·인카드 서식·전문검색·위키링크·붙여넣기 정제·빈 메모 정리·다중탭 동기화·a11y·자동링크)을 10 워커 1:1 분할. `MarkdownEditor.tsx`/`workspace.ts` 핫스팟을 P0 확장 슬롯화로 충돌 0.
> **메모/펜 후속** — 드로잉 엔진 단일화(`useDrawing`)·펜 모드 UX·마크다운 placeholder·죽은 코드 정리·전역 undo 스택. 펜 overlay 정렬은 고정폭 720px 컬럼 좌표계로 보장.
