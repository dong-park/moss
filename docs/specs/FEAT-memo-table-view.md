...
| D1 행 클릭 보드 복원 | 표 진입 시점 보드·뷰포트를 기억, 메모창 닫힘·캔버스 복귀 시 복원. 행 열기 보드 전환은 `lastOpenedAt` 미갱신. "캔버스에서 보기"는 의도적 이동이라 예외 | 행마다 보드가 바뀌면 표를 나갈 때 원래 맥락을 잃고 "최근 연 보드" 순서가 오염된다 |
| D2 되돌리기 | 표 다중 휴지통에만 "N개를 휴지통으로 보냈어요 · 되돌리기" 토스트. 복구는 기존 `storage.restoreNote` 재사용(연결선 포함) | 캔버스에는 토스트가 없지만 표는 다중 선택 삭제라 실수 복구 동선이 필요하다 |
