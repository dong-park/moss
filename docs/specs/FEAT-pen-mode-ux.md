...
## 0. 잔여 작업 (브라우저 QA 발견, 2026-06-05) — ✅ 해결

- **펜 모드 종료가 Esc 키 전용이던 문제** — 브라우저 실측에서 펜 도구 버튼 재클릭(2회 테스트)·툴바 닫기 모두 펜 모드를 끄지 못했고, `Canvas.tsx:156`상 Escape만 `setPenMode(false)`에 배선돼 있었음. Esc가 불편한 환경(태블릿·터치)에서 갇힐 수 있었음.
- **✅ fix 완료 (2026-06-05)**: `PenToolbar`에 「펜 모드 종료」 ✕ 버튼 추가(→ `setPenMode(false)`, Esc와 동일). i18n `workspace.pen.toolbar.exit` + 단위 테스트 1건. tsc 0·eslint 0·check-i18n 0.
